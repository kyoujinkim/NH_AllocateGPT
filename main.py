...
# define Universe
asset_dict = {
              'subasset1': ['국내주식', '선진국주식', '이머징주식'],
              'subasset4': ['성장주', '가치주'],
              'subasset7': ['반도체','은행','자동차','화학','증권','운송','바이오','소비재'],
              }
...
if __name__ == '__main__':
    ...
    for date in datearr:
        ...
        if os.path.isdir(f'C:/Users/NHWM/PycharmProjects/AllocateGPT/db/{date_str}'):
            db = mDB.readDB(dbPath=f'C:/Users/NHWM/PycharmProjects/AllocateGPT/db/{date_str}')
        else:
            pdfPath = mDB.filterPDF_byPeriod(s_date_str, date_str)
            subdocList = mDB.readPDF(pdfPath)
            db = mDB.makeDB(subdocList,
                            dbPath=f'C:/Users/NHWM/PycharmProjects/AllocateGPT/db/{date_str}')
